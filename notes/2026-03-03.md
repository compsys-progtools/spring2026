...
```{tip}
You should do *either* the `in groups` (if in class) or the `on your own` (if making up) sections below for the x.x.x. For the x.x sections do all of them.
```

```{important}
 Use the makeup workflow with an [ISO formated date](https://www.iso.org/iso-8601-date-and-time-format.html) (YYYY-MM-DD) if you work asynchronously.  If in class you can do a regular experience badge, but include the sections as directed . 
```
+++{"lesson_part": "main"}
...
Preview the [Stack Overflow Developer Survey](https://survey.stackoverflow.co/2025/technology/) Technology section parts that are about programming languages. 


Pick 2-3 points for discussion with your neighbors and write them in your experience badge.  These can be facts you found most interesting or questions you have based on the results (it can be clarifying or deeper questions). 

After discussing, add notes about if you all agreed or disagreed or if there was/not overlap in what you picked out. 
...
In class you can work on this on a whiteboard or on your computer. If on a whiteboard, take a picture and include that in your experience badge (by adding it to the repo as a normal file, not in the PR comment).
...
**Google docs can also export as markdown now**
```
````
...
Together, produce a list of questions for what other things you would want to know about how programming languages compare.  All of you should include the collaboratively developed list in your experience badge. 
