...
### Can a computer have more than 3 registers?

Yes! and they do, there are just 3 *roles* of registers in the CPU emulator we saw today. 

### Who choses or how does the computer know that that biniary is that instruction?

The hardware designers decide. 

### So say if you need 12 review and 12 practice badges for you grade goal, they cannot be from the same date? 

Correct, that would be 24 total different days. 

### How exactly does the jump decide where to go—like what determines the line it jumps to?

It jumps based on the address register value. 

### What are some topics in computing that assembly is used a lot for?

People don't *write* assembly much by hand (even before AI), but to optimize a compiler, or write a new one you have to write code that writes assembly. 


### Will a modern compiler always optimize things into the least cpu cycles, or does it depend on complexity


it depends on complexity and the compiler optimization settings. 